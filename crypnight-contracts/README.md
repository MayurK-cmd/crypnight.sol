...
- `forfeit`: End match, return stakes if one player forfeits
...
Compiled programs appear in `target/deploy/`.
...
- `authority` (signer) – Authority (backend)
- `system_program` – System program
...
- `duel_match` (writable) – Match account
- `duel_escrow` (writable) – Escrow PDA
- `forfeit_player` (signer) – Player forfeiting
...
| Issue | Cause | Fix |
|-------|-------|-----|
| Build fails: `anchor not found` | Anchor CLI not installed | `npm install -g @coral-xyz/anchor` |
| Deploy fails: `insufficient funds` | Payer account out of SOL | `solana airdrop 5` |
| Instruction fails: `account not signer` | Missing signer in accounts | Verify all (signer) accounts are signed |
